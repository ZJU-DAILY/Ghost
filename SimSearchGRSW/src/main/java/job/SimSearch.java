...
public class SimSearch {
    ...
    public static double gridLength;
    public static long gridlenRate;
    ...
    public static void main(String[] args) throws Exception {
        ...
        gridLength = ParamHelper.getGridLength();
        gridlenRate = 1;
        while (gridLength < 1 || (gridLength - (int) gridLength > 1e-7)) {
            gridLength *= 10;
            gridlenRate *= 10;
        }
        ...
        //run
        env.execute("Similarity Search with Grid-RTree Index");
    }
    ...
    public void apply(StreamExecutionEnvironment env) {
        ...
        SingleOutputStreamOperator<QueryTraInfo> queryTraInfoStream = pointStream.connect(queryInfoStream)
                ...
                .process(new QueryTraInfoGenerator(gridLength, gridlenRate, timeWindowSize))
        ...
        SingleOutputStreamOperator<QueryTraInfo> broadcastQueryTraInfoStream = queryTraInfoStream
                ...
                .flatMap(new QueryTraInfoBroadcaster(dataSize, dataSplitScale))
                .setParallelism(parallelism);
        SingleOutputStreamOperator<QueryPair> queryPairStream = broadcastQueryTraInfoStream.connect(pointStream)
                ...
                .process(new QueryPairGenerator(gridLength, gridlenRate, timeWindowSize))
                .setParallelism(parallelism);

        //rtree剪枝
        SingleOutputStreamOperator<QueryPair> prunedQueryPairStream = queryPairStream
                .keyBy(pair -> pair.queryTra.id)
//                .process(new RTreePruner(timeWindowSize, distMeasure))
                .process(new SimplifyRTreePruner(timeWindowSize, distMeasure))
                .setParallelism(parallelism);

        //打时间戳
        prunedQueryPairStream = prunedQueryPairStream
                .map(pair -> {
        ...
        SingleOutputStreamOperator<QueryPair> calculatedQueryPairStream = prunedQueryPairStream
                //根据<t1.id,t2.id>分区
