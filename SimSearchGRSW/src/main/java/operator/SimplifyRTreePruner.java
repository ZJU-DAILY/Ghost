...
public class SimplifyRTreePruner extends KeyedProcessFunction<Long, QueryPair, QueryPair> {
    public long timeWindow;
    public Measure measure;

    public SimplifyRTreePruner(long timeWindow, Measure measure) {
    ...
    public void processElement(QueryPair pair, KeyedProcessFunction<Long, QueryPair, QueryPair>.Context ctx, Collector<QueryPair> out) throws Exception {
        ...
        if (!loaded) {
            ...
            if (rtree.root().isPresent()) {
                mbr = findContainedMbr(rtree, queryTra, pair.threshold, rtree.root().get());
            }
            mbrState.update(mbr);
        }
        //加载anotherTra到rtree
